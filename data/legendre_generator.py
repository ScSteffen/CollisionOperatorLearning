...
parser.add_argument("--integration_order", default=100, type=int, help="Quadrature order")
...
for i in tqdm(range(num_train)):
    data_f.append(f_legendre(quad_pts, quad_weights, polynomial_order))
    data_Q.append(Q.evaluate_Q(data_f[-1]))

np.savez(
    os.path.join(PATH, "legendre_train_data.npz"),
...
for i in tqdm(range(num_test)):
    data_f.append(f_legendre(quad_pts, quad_weights))
    data_Q.append(Q.evaluate_Q(data_f[-1]))

np.savez(
    os.path.join(PATH, "legendre_test_data.npz"),
    data_f=np.array(data_f),
    data_Q=np.array(data_Q),
)
