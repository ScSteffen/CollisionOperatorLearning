...
def run_neural_simulation(
        f_init, n_x, n_q, n_t, dt, dx, sigma_s, sigma_t, source, omega, model, grid, w_q
):
    f_kin = np.copy(f_init)
    fluxes = np.zeros(shape=(n_q, n_x))
...
def main():
    ...
    f_init = epsilon * np.ones((n_q, n_x))
    f_init[:, 0] = 1.0  # Left boundary (Dirichlet boundary condition)
    f_init[:, -1] = epsilon  # Right boundary (Dirichlet boundary condition)

    # Define the time step and number of time steps
    dt = 0.9 * dx  # obeys cfl
    n_t = 50

    f_tN_classic = run_classic_simulation(f_init, n_x, n_q, n_t, dt, dx, sigma_s, sigma_t, source, omega, Q)
    ...
    gparams = {
        ...
        "dimension": 1,
        "data_file": "legendre",
        "integration_order": n_q,
        "model": "deeponet",
        "branch_hidden": [n_q, 50, 50, 8],
        "trunk_hidden": [1, 50, 50, 8],
        "act": "tanh",
        "d_out": 1,
        "use_bias": "no",
        "use_gram": False,
    ...
    f_res_tn_classic = w_q @ f_tN_classic / 2.0
    ...
    plt.plot(np.linspace(0, L, n_x), f_res_tn_classic)
    plt.xlabel("Position (cm)")
    plt.ylabel("Angular Flux")
    plt.title("1D Discrete Ordinates with Dirichlet Boundary Conditions")
    plt.legend(["t0", "tN", "tN_classic"])
    plt.grid(True)
    plt.savefig("1d_inflow_HG_" + str(g) + ".png")
    return 0
