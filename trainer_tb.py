...
# python3 trainer_tb.py wo_bias --seed 0 --gpu 0 --data_file entropy --dimension 1 --integration_order 100 --model deeponet --branch_hidden 100 8 8 8 --trunk_hidden 1 8 8 8 --use_bias no --epochs 100000 --lambda 0

## vanila DeepONet w/ bias
# python3 trainer_tb.py w_bias --seed 0 --gpu 0 --data_file entropy --dimension 1 --integration_order 100 --model deeponet --branch_hidden 100 8 8 8 --trunk_hidden 1 8 8 8 --use_bias vanila --epochs 100000 --lambda 0

## (soft constraint) DeepONet with additional orthogonal loss
# python3 trainer_tb.py soft_lamb --seed 0 --gpu 0 --data_file entropy --dimension 1 --integration_order 100 --model deeponet --branch_hidden 100 8 8 8 --trunk_hidden 1 8 8 8 --use_bias vanila --epochs 100000 --lambda 0.1

## (Hard constraint) DeepONet with gram schmidt for basis
# python3 trainer_tb.py hard_gram --seed 0 --gpu 0 --data_file entropy --dimension 1 --integration_order 100 --model deeponet --branch_hidden 100 8 8 8 --trunk_hidden 1 8 8 8 --use_bias no --use_gram --epochs 100000 --lambda 0

## (Hard constraint) DeepONet with special bias
# python3 trainer_tb.py hard_special --seed 0 --gpu 0 --data_file entropy --dimension 1 --integration_order 100 --model deeponet --branch_hidden 100 8 8 8 --trunk_hidden 1 8 8 8 --use_bias depend --epochs 100000 --lambda 0
...
parser.add_argument(
    "--trunk_hidden", default=[1, 8, 8, 8], nargs="+", type=int, help="trunk network"
)
...
parser.add_argument(
    "--use_bias", type=str, choices=["no", "vanila", "depend"], required=True
)
...
train_data_f, train_data_Q = torch.FloatTensor(train_data["data_f"]), torch.FloatTensor(
    train_data["data_Q"]
)
test_data_f, test_data_Q = torch.FloatTensor(test_data["data_f"]), torch.FloatTensor(
    test_data["data_Q"]
)
...
if dimension == 1:
    ...
    grid = (
        torch.FloatTensor(Q.get_quad_pts().transpose(1, 2, 0))
        .reshape(-1, dimension)
        .cuda()
    )
...
if use_sche:
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=step_size_sche, gamma=lr_sche
    )
...
for epoch in tqdm(range(1, num_epochs + 1)):
    ...
    if epoch % 50 == 0:
        ...
        pbar.set_description(
            "###### Epoch : %d, Loss_train : %.8f, Loss_ortho : %.8f, rel_train : %.5f, rel_test : %.5f ######"
            % (
                epoch,
                list_train_loss_deeponet[-1],
                list_train_loss_ortho[-1],
                list_train_Q_rel_error[-1],
                list_test_Q_rel_error[-1],
            )
        )
