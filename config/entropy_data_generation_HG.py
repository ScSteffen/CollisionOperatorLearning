...
for i in tqdm(range(num_train)):
    if dimension==1:
        ...
        data_Q.append(Q.evaluate_Q(f_train.T[i]))
np.savez(os.path.join(PATH, 'entropy_HG_train_data.npz'),data_f=np.array(f_train.T), data_Q=np.array(data_Q))
...
np.savez(os.path.join(PATH, 'entropy_HG_test_data.npz'),data_f=np.array(f_test.T), data_Q=np.array(data_Q))